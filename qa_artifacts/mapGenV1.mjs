...
const tolPctRaw = remaskCfg?.land_target_tolerance_pct ?? config?.scale?.realm_hexes_land_target_tolerance_pct;
  ...
  const borderReshave = () => {
    ...
    // Penalize locally-straight borders so reshave prefers scalloping/turns.
    // Local straightness proxy: count opposite-direction selected-neighbor pairs.
    const straightPenaltyWeight = Math.max(0, Math.floor(Number(remaskCfg?.reshave_straight_penalty ?? 2500)));
    const straightPairsIfSelected = (i, assumeSelected) => {
      // If we're evaluating a frontier tile, we treat it as selected for this computation.
      let pairs = 0;
      const has = [0,0,0,0,0,0];
      for (let d = 0; d < 6; d++) {
        const ni = neighbors[i*6 + d];
        if (ni < 0) continue;
        if (selected[ni] === 1) has[d] = 1;
      }
      // Opposite directions: (0,3), (1,4), (2,5)
      if (has[0] && has[3]) pairs++;
      if (has[1] && has[4]) pairs++;
      if (has[2] && has[5]) pairs++;
      // Frontier tiles (assumeSelected=true) tend to add one more "straight" pair only
      // if they connect two opposite neighbors; this is enough to discourage extending facets.
      return pairs;
    };
    ...
    for (let it = 0; it < phaseA_iters; it++) {
      ...
      const scoreAdd = (i) => dist[i] + straightPenaltyWeight * straightPairsIfSelected(i, true);
      frontier.sort((a, b) => (scoreAdd(a) - scoreAdd(b)) || (a - b));
    ...
    for (let it = 0; it < phaseB_iters; it++) {
      ...
      const scoreDel = (i) => dist[i] + straightPenaltyWeight * straightPairsIfSelected(i, false);
      boundary.sort((a, b) => (scoreDel(b) - scoreDel(a)) || (a - b));
...
const worldBufferPct = Math.max(0, Number(config?.mapgen?.world_radius_buffer_pct ?? 0.15));
const worldRadius = Math.ceil(coreRadius * (1 + worldBufferPct)) + worldMargin;
