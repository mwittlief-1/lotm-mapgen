import { clampInt, asNonNegInt } from "../sim/util.js";
import { hasImprovement } from "./improvements.js";
function ensureMods(state) {
    const f = state.flags;
    if (!f._mods || typeof f._mods !== "object")
        f._mods = {};
    return f._mods;
}
function addMod(state, key, value) {
    const mods = ensureMods(state);
    mods[key] = (mods[key] ?? 1) * value;
}
function addBushels(state, delta) {
    state.manor.bushels_stored = asNonNegInt(state.manor.bushels_stored + Math.trunc(delta));
}
function addCoin(state, delta) {
    state.manor.coin = asNonNegInt(state.manor.coin + Math.trunc(delta));
}
function addUnrest(state, delta) {
    state.manor.unrest = clampInt(state.manor.unrest + Math.trunc(delta), 0, 100);
}
function addPopulation(state, delta) {
...
function baseWeight(w, notes = []) {
    return { weight: Math.max(0, w), notes };
}
function wWithImprovement(state, base, ifHas, mult, note) {
...
export const EVENT_DECK = [
    ...
    // --- Military / war levy ---
    {
        ...
        apply: (s, rng) => {
            if (s.manor.obligations.war_levy_due)
                return ["A levy is already outstanding."];
    ...
    // --- Construction / project volatility ---
    {
        ...
        apply: (s, rng) => {
            if (!s.manor.construction)
                return ["No active works; the mason moves on."];
    ...
    },
    {
        ...
        apply: (s, rng) => {
            if (!s.manor.construction)
                return ["No active works are affected."];
...
const FLAVOR = [
    ...
    { id: "evt_small_theft_tools", title: "Tools Misplaced", category: "construction", cooldown: 7, w: 0.16, apply: (s, r) => { if (!s.manor.construction)
            return ["Nothing is misplaced."]; const pen = r.int(3, 8); s.manor.construction.progress = asNonNegInt(Math.max(0, s.manor.construction.progress - pen)); return [`Delays at the works: -${pen} progress.`]; } },
    { id: "evt_extra_hands", title: "Extra Hands Volunteer", category: "construction", cooldown: 7, w: 0.16, apply: (s, r) => { if (!s.manor.construction)
            return ["No active works; hands disperse."]; const bon = r.int(3, 10); s.manor.construction.progress = asNonNegInt(s.manor.construction.progress + bon); return [`Work quickens: +${bon} progress.`]; } },
