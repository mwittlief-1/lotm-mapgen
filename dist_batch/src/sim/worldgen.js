...
function ensureRelationshipEdgesToPlayerHead(state, extHouseIds) {
    ...
    for (const hid of [...extHouseIds].sort()) {
        const h = houses?.[hid];
        const headId = (h && typeof h === "object" && typeof h.head_id === "string" && h.head_id) ? h.head_id : hid.replace(/^h_ext_/, "p_ext_") + "_head";
        if (!headId)
            continue;
...
export function ensureExternalHousesSeed_v0_2_2(state) {
    ...
    for (let i = 1; i <= houseCount; i++) {
        ...
        const tier = hRng.pick(HOUSE_TIERS);
        ...
        const childIds = [];
        if (childCount >= 1) {
            const sex1 = hRng.bool(0.55) ? "M" : "F";
            const age1 = hRng.int(1, Math.max(1, headAge - 16));
            if (!people[child1Id])
                people[child1Id] = mkPerson(hRng.fork(`person:${child1Id}`), child1Id, sex1, age1, surname, false);
            childIds.push(child1Id);
        }
        if (childCount >= 2) {
            const sex2 = hRng.bool(0.55) ? "M" : "F";
            const age2 = hRng.int(1, Math.max(1, headAge - 16));
            if (!people[child2Id])
                people[child2Id] = mkPerson(hRng.fork(`person:${child2Id}`), child2Id, sex2, age2, surname, false);
